...
export async function checkTokenBudget(userId, plan) {
  const limits = PLAN_LIMITS[plan] ?? PLAN_LIMITS.free
  const today = new Date().toISOString().split("T")[0]

  let { data: usage } = await supabase
    .from("user_usage")
    .select("tokens_used, usage_date")
    .eq("user_id", userId)
    .single()
  ...
  if (!usage || usage.usage_date !== today) {
    const { data: reset } = await supabase
      .from("user_usage")
      .upsert({ user_id: userId, tokens_used: 0, usage_date: today })
      .select("tokens_used, usage_date")
      .single()
    usage = reset
  }
  ...
  const limit = limits.dailyTokens
